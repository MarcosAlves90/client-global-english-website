...
export function useCourseManagementActions({
  ...
  const handleUpdateMaterial = React.useCallback(
    async (form: { id: string; title: string; markdown: string; attachments?: NonNullable<Material["attachments"]> }) => {
      if (!user) return false
      try {
        const idToken = await user.getIdToken()
  ...
  const handleUpdateActivityAttachments = React.useCallback(
    async (activityId: string, attachments: Activity["attachments"] = []) => {
      if (!user) return false
      try {
        const idToken = await user.getIdToken()
        await updateAdminActivity(idToken, { id: activityId, attachments })
        toast.success("Anexos da atividade atualizados")
        await loadActivities(true)
        return true
      } catch {
        toast.error("Erro ao atualizar anexos da atividade")
        return false
      }
    },
    [loadActivities, user]
